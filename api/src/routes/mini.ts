...
interface MiniJWTPayload {
  userId: number
  openid: string
}

async function verifyMiniUser(
  headers: Record<string, string | undefined>,
  miniJwtPlugin: { verify: (token: string) => Promise<unknown> }
) {
  const authorization = headers.authorization
  if (!authorization?.startsWith('Bearer ')) return null
  const token = authorization.slice(7)
  try {
    return (await miniJwtPlugin.verify(token)) as MiniJWTPayload | null
  } catch {
    return null
  }
}
...
export const miniRoutes = new Elysia({ prefix: '/mini' })
  ...
  .get(
    ...
    async ({ headers, query, miniJwt }) => {
      ...
      const authUser = await verifyMiniUser(headers, miniJwt)
      if (!authUser?.userId) {
  ...
  .get(
    ...
    async ({ headers, query, miniJwt }) => {
      ...
      const result = orderList.map((row) => ({
        ...
        couponDiscount: Number(row.orders.couponDiscount),
        promotionDiscount: Number(row.orders.promotionDiscount),
  ...
  .get(
    ...
    async ({ headers, params, miniJwt }) => {
      ...
      return success({
        ...
        couponDiscount: Number(order.orders.couponDiscount),
        promotionDiscount: Number(order.orders.promotionDiscount),
