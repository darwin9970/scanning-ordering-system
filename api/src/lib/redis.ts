...
try {
  const testRedis = (globalThis as unknown as { __TEST_REDIS__?: Redis }).__TEST_REDIS__
...
export function getRedis() {
  const maybeTestRedis = (globalThis as unknown as { __TEST_REDIS__?: Redis }).__TEST_REDIS__
  if (process.env.NODE_ENV === 'test' && maybeTestRedis) {
    return maybeTestRedis
  }
  return redis
}
