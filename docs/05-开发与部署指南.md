# 桌台扫码点单系统 - 开发与部署指南

## 一、开发环境搭建

### 1.1 系统要求

| 软件       | 版本要求 | 说明               |
| :--------- | :------- | :----------------- |
| Bun        | 1.x      | 包管理器和运行时   |
| PostgreSQL | 16+      | 主数据库           |
| Redis      | 7.0+     | 缓存和消息队列     |
| Docker     | 20.x+    | 容器化部署（可选） |
| Git        | 2.x      | 版本控制           |

### 1.2 技术栈

| 层级     | 技术栈                                        | 说明                          |
| :------- | :-------------------------------------------- | :---------------------------- |
| 后台管理 | Next.js 16, TailwindCSS, React Query, Zustand | App Router, Server Components |
| API 服务 | Elysia, Drizzle ORM                           | 高性能 API，类型安全 ORM      |
| 消息队列 | Redis Stream                                  | 打印任务队列                  |
| 实时通信 | WebSocket                                     | 订单实时通知                  |
| 数据库   | PostgreSQL 16                                 | 主数据库                      |
| 缓存     | Redis 7                                       | 会话管理、共享购物车          |

### 1.3 安装 Bun

```bash
# Windows (PowerShell)
powershell -c "irm bun.sh/install.ps1 | iex"

# macOS / Linux
curl -fsSL https://bun.sh/install | bash
```

### 1.4 安装 PostgreSQL

下载地址: https://www.postgresql.org/download/

创建数据库:

```sql
CREATE DATABASE qr_order;
```

### 1.5 安装 Redis

- Windows: https://github.com/tporadowski/redis/releases
- macOS: `brew install redis`
- Linux: `apt install redis-server`

## 二、项目启动

### 2.1 克隆项目

```bash
cd your-workspace
# 项目已在本地创建
```

### 2.2 API 服务启动

```bash
cd api

# 1. 安装依赖
bun install

# 2. 复制环境变量
cp .env.example .env

# 3. 编辑 .env 文件
# DATABASE_URL="postgres://postgres:password@localhost:5432/qr_order"
# REDIS_URL="redis://localhost:6379"
# JWT_SECRET="your-secret-key"

# 4. 同步数据库结构
bun run db:push

# 5. 播种测试数据
bun run db:seed

# 6. 启动开发服务器
bun run dev

# 7. (可选) 启动打印 Worker
bun run worker:dev
```

API 服务运行在: http://localhost:4000
WebSocket: ws://localhost:4000/ws/store/:storeId

### 2.3 后台管理系统启动

```bash
cd admin

# 1. 安装依赖
bun install

# 2. 启动开发服务器
bun run dev
```

后台管理系统运行在: http://localhost:3000

### 2.4 验证安装

1. 访问 http://localhost:4000/swagger 查看 API 文档
2. 访问 http://localhost:3000/login 进入登录页
3. 使用测试账号登录: admin / admin123

## 三、开发流程

### 3.1 目录约定

```
api/
├── src/
│   ├── db/schema.ts      # Drizzle 数据模型
│   ├── routes/           # API 路由
│   ├── types/            # TypeScript 类型定义
│   ├── ws/               # WebSocket 模块
│   ├── worker/           # Worker 服务
│   └── services/         # 业务服务

admin/
├── app/dashboard/        # 页面
├── components/           # 组件
├── lib/api.ts            # API 客户端
├── types/                # 类型定义
└── store/                # Zustand 状态
```

### 3.2 添加新功能流程

**后端:**

1. 修改 `src/db/schema.ts` 添加数据模型
2. 运行 `bun run db:push` 同步数据库
3. 在 `src/routes/` 创建路由文件
4. 在 `src/types/index.ts` 添加类型定义
5. 在 `src/index.ts` 注册路由

**前端:**

1. 在 `types/index.ts` 添加对应类型
2. 在 `lib/api.ts` 添加 API 方法
3. 在 `app/dashboard/` 创建页面
4. 在 `components/layout/sidebar.tsx` 添加菜单

### 3.3 常用命令

**API 项目:**

```bash
bun run dev          # 启动开发服务器
bun run start        # 生产模式启动
bun run worker       # 启动打印 Worker
bun run db:push      # 同步数据库结构
bun run db:generate  # 生成迁移文件
bun run db:studio    # 打开 Drizzle Studio
bun run db:seed      # 播种测试数据
```

**Admin 项目:**

```bash
npm run dev          # 启动开发服务器
npm run build        # 构建生产版本
npm run start        # 启动生产服务器
npm run lint         # ESLint 检查
npm run lint:fix     # ESLint 自动修复
npm run typecheck    # TypeScript 类型检查
npm run format       # Prettier 格式化
```

## 四、代码质量工具

项目配置了完整的代码质量工具链，确保代码规范一致。

### 4.1 工具概览

| 工具         | 作用             | 配置文件            |
| :----------- | :--------------- | :------------------ |
| ESLint       | 代码规范检查     | `eslint.config.mjs` |
| Prettier     | 代码格式化       | `.prettierrc`       |
| Husky        | Git hooks 管理   | `.husky/`           |
| lint-staged  | 只检查暂存文件   | `package.json`      |
| EditorConfig | 跨编辑器格式统一 | `.editorconfig`     |

### 4.2 提交前自动检查

每次 `git commit` 时会自动运行以下检查：

```
git commit -m "feat: xxx"
  │
  ├── 📦 Admin 项目
  │   ├── lint-staged (ESLint + Prettier)
  │   └── tsc --noEmit (TypeScript 检查)
  │
  ├── 📦 API 项目
  │   ├── lint-staged (ESLint + Prettier)
  │   └── tsc --noEmit (TypeScript 检查)
  │
  └── ✅ 通过则提交成功
```

如果检查失败，提交会被阻止，需要先修复错误。

### 4.3 编辑器配置 (VSCode)

项目包含 `.vscode/settings.json`，安装推荐扩展后支持：

- **保存时自动格式化**: Prettier 格式化代码
- **保存时自动修复**: ESLint 自动修复可修复的问题
- **TypeScript 智能提示**: 使用项目内的 TypeScript 版本

**推荐扩展** (`.vscode/extensions.json`):

- Prettier - Code formatter
- ESLint
- Tailwind CSS IntelliSense
- GitLens

### 4.4 手动运行检查

```bash
# Admin 项目
cd admin
npm run lint           # 检查 ESLint
npm run lint:fix       # 自动修复
npm run typecheck      # TypeScript 检查
npm run format         # Prettier 格式化

# API 项目
cd api
bun run lint           # 检查 ESLint
bun run lint:fix       # 自动修复
bun run typecheck      # TypeScript 检查
bun run format         # Prettier 格式化
```

### 4.5 跳过检查（不推荐）

如果需要临时跳过 pre-commit 检查：

```bash
git commit -m "fix: xxx" --no-verify
```

⚠️ 请仅在紧急情况下使用，正常开发应确保代码通过所有检查。

## 五、Docker 部署

### 5.1 前置要求

- Docker 20.10+
- Docker Compose 2.0+
- 至少 2GB 可用内存
- 至少 10GB 可用磁盘空间

### 5.2 快速部署

**方式一：使用启动脚本（推荐）**

```bash
# Linux/macOS
chmod +x docker-start.sh
./docker-start.sh

# Windows PowerShell
.\docker-start.ps1
```

**方式二：手动部署**

```bash
# 1. 复制环境变量（如果不存在）
cp .env.example .env

# 2. 编辑 .env 设置密码（生产环境请修改）
# POSTGRES_PASSWORD=your_password
# JWT_SECRET=your_jwt_secret
# NEXT_PUBLIC_API_URL=http://localhost:4000
# NEXT_PUBLIC_WS_URL=ws://localhost:4000

# 3. 启动所有服务
docker-compose up -d

# 4. 等待服务启动（约 20-30 秒）
# 检查服务状态
docker-compose ps

# 5. 初始化数据库（使用 --force 自动确认，无需交互）
docker-compose exec -T api bunx drizzle-kit push --force

# 6. 初始化种子数据（可选）
docker-compose exec -T api bun run db:seed

# 7. 初始化页面配置（可选）
docker-compose exec -T api bun run init:page-configs
```

**重新部署（代码更新后）**

```bash
# 1. 停止并删除旧容器
docker-compose down

# 2. 重新构建镜像（代码更新后需要）
docker-compose build --no-cache api worker admin

# 3. 启动所有服务
docker-compose up -d

# 4. 检查服务状态
docker-compose ps

# 5. 查看日志确认服务正常
docker-compose logs -f api
```

### 5.3 服务说明

| 服务     | 端口 | 说明              | 运行时 |
| :------- | :--- | :---------------- | :----- |
| postgres | 5432 | PostgreSQL 数据库 | -      |
| redis    | 6379 | Redis 缓存        | -      |
| api      | 4000 | API 服务          | Bun    |
| worker   | -    | 打印任务 Worker   | Bun    |
| admin    | 3000 | 后台管理系统      | Bun    |

### 5.4 常用命令

**服务管理**

```bash
# 查看服务状态
docker-compose ps

# 查看日志
docker-compose logs -f              # 所有服务
docker-compose logs -f api          # API 服务
docker-compose logs -f admin        # Admin 服务
docker-compose logs -f worker       # Worker 服务

# 重启服务
docker-compose restart api
docker-compose restart admin
docker-compose restart worker

# 停止服务
docker-compose down

# 停止并删除数据卷（⚠️ 会删除数据库数据）
docker-compose down -v

# 重新构建并启动
docker-compose up -d --build

# 进入容器
docker-compose exec api sh
docker-compose exec admin sh
```

**数据库操作**

```bash
# 初始化/更新数据库结构（自动确认）
docker-compose exec -T api bunx drizzle-kit push --force

# 初始化测试数据
docker-compose exec -T api bun run db:seed

# 初始化页面配置
docker-compose exec -T api bun run init:page-configs

# 打开 Drizzle Studio（数据库可视化工具）
docker-compose exec api bunx drizzle-kit studio
```

**镜像管理**

```bash
# 重新构建特定服务镜像
docker-compose build --no-cache api      # API 服务
docker-compose build --no-cache worker   # Worker 服务
docker-compose build --no-cache admin    # Admin 服务

# 重新构建所有服务镜像
docker-compose build --no-cache

# 查看镜像
docker images | grep test
```

### 5.5 数据持久化

数据存储在 Docker volumes 中:

- `postgres_data`: 数据库文件
- `redis_data`: Redis 数据
- `api_uploads`: API 上传文件

### 5.6 访问地址

| 服务         | 地址                          |
| :----------- | :---------------------------- |
| 后台管理系统 | http://localhost:3000        |
| API 服务     | http://localhost:4000        |
| API 文档     | http://localhost:4000/swagger |
| 数据库       | localhost:5432                |
| Redis        | localhost:6379                |

### 5.7 故障排查

**服务无法启动**

```bash
# 检查端口占用
netstat -tulpn | grep -E '3000|4000|5432|6379'

# 查看服务日志
docker-compose logs api
docker-compose logs admin

# 检查容器状态
docker-compose ps
```

**数据库连接失败**

```bash
# 检查数据库健康状态
docker-compose exec postgres pg_isready -U postgres

# 检查环境变量
docker-compose exec api env | grep DATABASE_URL

# 测试数据库连接
docker-compose exec postgres psql -U postgres -d qr_order -c "SELECT COUNT(*) FROM stores;"

# 重新初始化数据库（如果数据库结构有问题）
docker-compose exec -T api bunx drizzle-kit push --force
```

**API 无法访问**

```bash
# 测试 API 健康检查
curl http://localhost:4000/health

# 检查 API 服务日志
docker-compose logs api

# 检查服务状态
docker-compose ps api

# 检查 CORS 配置（API 需要允许前端域名）
# 查看 api/src/index.ts 中的 CORS 配置

# 重启 API 服务
docker-compose restart api
```

**前端无法连接 API**

```bash
# 1. 确认 API 服务正常运行
curl http://localhost:4000/health

# 2. 检查 API 服务日志
docker-compose logs api

# 3. 检查环境变量配置
docker-compose exec admin env | grep NEXT_PUBLIC_API_URL

# 4. 重启 Admin 服务（如果环境变量已更新）
docker-compose restart admin
```

其他检查项：
1. 检查 `NEXT_PUBLIC_API_URL` 配置是否正确
2. 检查浏览器控制台错误信息
3. 检查 Network 标签页的请求详情
4. 确认 API 服务正常运行：`curl http://localhost:4000/health`

### 5.8 生产环境部署

**使用 Nginx 反向代理**

```bash
# 启用 Nginx 服务
docker-compose --profile production up -d nginx
```

**配置 SSL 证书**

1. 将 SSL 证书放置到 `nginx/ssl/` 目录
2. 修改 `nginx/nginx.conf` 启用 HTTPS 配置
3. 重启 Nginx 服务

**数据备份**

```bash
# 备份数据库
docker-compose exec postgres pg_dump -U postgres qr_order > backup.sql

# 恢复数据库
docker-compose exec -T postgres psql -U postgres qr_order < backup.sql

# 备份 Redis 数据
docker-compose exec redis redis-cli SAVE
docker cp qr-order-redis:/data/dump.rdb ./redis-backup.rdb
```

### 5.9 安全建议

1. **修改默认密码**：生产环境必须修改 `POSTGRES_PASSWORD` 和 `JWT_SECRET`
2. **使用 HTTPS**：生产环境必须配置 SSL 证书
3. **限制端口暴露**：仅暴露必要的端口
4. **定期更新**：定期更新 Docker 镜像和依赖
5. **数据备份**：定期备份数据库和重要文件

## 六、环境变量说明

### 6.1 Docker Compose 环境变量

| 变量名            | 必填 | 说明         | 默认值      |
| :---------------- | :--- | :----------- | :---------- |
| POSTGRES_USER     | 否   | 数据库用户   | postgres    |
| POSTGRES_PASSWORD | 是   | 数据库密码   | postgres123 |
| POSTGRES_DB       | 否   | 数据库名     | qr_order    |
| JWT_SECRET        | 是   | JWT 签名密钥 | -           |

### 6.2 API 环境变量

| 变量名       | 必填 | 说明                  | 示例                                        |
| :----------- | :--- | :-------------------- | :------------------------------------------ |
| DATABASE_URL | 是   | PostgreSQL 连接字符串 | postgres://user:pwd@localhost:5432/qr_order |
| REDIS_URL    | 否   | Redis 连接字符串      | redis://localhost:6379                      |
| JWT_SECRET   | 是   | JWT 签名密钥          | your-super-secret-key                       |
| PORT         | 否   | 服务端口              | 4000                                        |
| NODE_ENV     | 否   | 环境模式              | development / production                    |

### 6.3 Admin 环境变量

| 变量名              | 必填 | 说明           | 示例                  |
| :------------------ | :--- | :------------- | :-------------------- |
| NEXT_PUBLIC_API_URL | 否   | API 地址       | http://localhost:4000 |
| NEXT_PUBLIC_WS_URL  | 否   | WebSocket 地址 | ws://localhost:4000   |

## 七、监控与运维

### 7.1 健康检查

```bash
# API 健康检查
curl http://localhost:4000/health

# 响应
{
  "status": "ok",
  "timestamp": "2024-01-01T00:00:00.000Z"
}
```

### 7.2 日志查看

```bash
# Docker 日志
docker-compose logs -f api
docker-compose logs -f admin

# 开发环境
# 控制台直接输出
```

### 7.3 数据库备份

```bash
# 导出数据库 (Docker)
docker-compose exec postgres pg_dump -U postgres qr_order > backup.sql

# 导入数据库
docker-compose exec -T postgres psql -U postgres qr_order < backup.sql
```

## 六、常见问题

### Q1: 数据库连接失败

检查:

- PostgreSQL 服务是否启动
- DATABASE_URL 格式是否正确
- 数据库用户权限

### Q2: Redis 连接失败

检查:

- Redis 服务是否启动 (可选，不影响核心功能)
- REDIS_URL 格式是否正确
- 防火墙端口

### Q3: 登录后跳转异常

检查:

- JWT_SECRET 是否配置
- Token 是否正确存储
- 浏览器 localStorage

### Q4: API 跨域问题

检查 API 的 CORS 配置:

```typescript
cors({
  origin: ["http://localhost:3000", "http://127.0.0.1:3000"],
  credentials: true,
});
```

### Q5: Docker 部署后接口不通

```bash
# 1. 检查服务是否正常启动
docker-compose ps

# 2. 检查服务日志
docker-compose logs api

# 3. 测试 API 健康检查
curl http://localhost:4000/health

# 4. 检查环境变量配置
docker-compose exec api env | grep DATABASE_URL
docker-compose exec api env | grep REDIS_URL

# 5. 确认数据库已初始化
docker-compose exec -T api bunx drizzle-kit push --force

# 6. 重启服务
docker-compose restart api
```
