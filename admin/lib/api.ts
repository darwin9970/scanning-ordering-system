...
class ApiClient {
  ...
  getDashboardOverview(storeId?: number): Promise<DashboardOverview> {
    return this.request<DashboardOverview>(`/api/dashboard/overview${this.buildQuery({ storeId })}`)
  }
  ...
  async upload(file: File): Promise<{ url: string }> {
    ...
    const res = await fetch(`${this.baseUrl}/api/upload/image`, {
      method: 'POST',
      headers,
      body: formData
    })
  ...
  getPageTypes(): Promise<Array<{ value: string; label: string }>> {
    return this.request<Array<{ value: string; label: string }>>('/api/page-configs/page-types')
  }

  getTabBarConfig(params: { storeId?: number }): Promise<{
    color: string
    ...
    return this.request(`/api/page-configs/tabbar${this.buildQuery(params)}`)
  }

  saveTabBarConfig(data: {
    storeId?: number
    tabBarConfig: {
      color: string
      selectedColor: string
    ...
    return this.request<PageConfig>('/api/page-configs/tabbar', {
      method: 'PUT',
      body: data
    })
  }
